...
import { and, desc, eq, gte, not, isNull, isNotNull } from "drizzle-orm";
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(handleWithFollowers.map((handle) => handle.url).length);
...
//           const analysis = analysisSchema.parse(rawAnalysis);
          ...
          const [profile] = await db.select().from(profiles).where(eq(profiles.handle_id, handle.id)).limit(1);
          ...
          }).where(eq(profiles.handle_id, handle.id));
